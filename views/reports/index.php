...
const DEMO_L = (function(){
    const d=[], n=new Date();
    for(let i=6;i>=0;i--){
        const t=new Date(n); t.setDate(n.getDate()-i);
        d.push(t.toLocaleDateString('en-EG',{month:'short',day:'numeric'}));
    }
    return d;
})();
const DEMO_DATASETS = [
        ...
        options: {
            ...
            plugins: {
                legend: {
                    ...
                    labels  : { color: '#8b9ab0', font: { size: 11 }, boxWidth: 10, padding: 14, usePointStyle: true }
                },
                tooltip: {
                    backgroundColor: '#1a2234', borderColor: '#253549', borderWidth: 1,
                    titleColor: '#f1f5f9', bodyColor: '#8b9ab0', padding: 10, cornerRadius: 8
                }
            },
            scales: {
                x: { ticks: { color: '#4a5568' }, grid: { color: '#1e2d40' } },
                y: { ticks: { color: '#4a5568' }, grid: { color: '#1e2d40' }, beginAtZero: true }
            }
        }
    });
}
...
async function loadChart(type) {
    ...
    const wrap = document.getElementById('reportChart')?.parentElement;
    if (wrap) wrap.style.opacity = '0.5';

    let labels   = DEMO_L;
    let datasets = DEMO_DATASETS;
    ...
    try {
        ...
        // Check if real data exists and has non-zero values
        const hasData = Array.isArray(data.labels)
            && data.labels.length > 0
            && Array.isArray(data.datasets)
            && data.datasets.some(function(d){ return d.data && d.data.some(function(v){ return v > 0; }); });
        ...
        console.warn('[SmartHome] Chart fetch failed, using demo data:', err.message);
    }

    if (wrap) wrap.style.opacity = '1';
    _drawChart(labels, datasets, type);
}
...
document.getElementById('rLineBtn')?.addEventListener('click', function() { loadChart('line'); });
document.getElementById('rBarBtn')?.addEventListener('click',  function() { loadChart('bar');  });

// Budget donut
(function(){
    var ctx = document.getElementById('budgetDonut');
    if (!ctx) return;
    var bd = <?= json_encode(array_map(function($b){ return ['label'=>ucfirst($b['resource_type']),'spent'=>(float)$b['current_spent'],'limit'=>(float)$b['monthly_limit']]; }, $budgets ?: [['resource_type'=>'electricity','current_spent'=>187.4,'monthly_limit'=>500],['resource_type'=>'water','current_spent'=>32.1,'monthly_limit'=>80],['resource_type'=>'gas','current_spent'=>54.8,'monthly_limit'=>120]])) ?>;
    new Chart(ctx, {
        type : 'doughnut',
        data : {
            labels  : bd.map(function(b){ return b.label; }),
            datasets: [{
                data           : bd.map(function(b){ return b.spent; }),
                backgroundColor: ['rgba(251,191,36,.7)','rgba(34,211,238,.7)','rgba(251,146,60,.7)'],
        ...
        options: {
            ...
            cutout             : '70%',
            plugins: {
                legend : { position:'bottom', labels:{ color:'#8b9ab0', font:{size:10}, boxWidth:8, padding:12, usePointStyle:true }},
                tooltip: {
                    backgroundColor:'#1a2234', borderColor:'#253549', borderWidth:1,
                    titleColor:'#f1f5f9', bodyColor:'#8b9ab0', padding:9, cornerRadius:8,
                    callbacks:{ label: function(c){ return c.label+': '+c.parsed.toFixed(2)+' EGP'; }}
                }
            }
        }
    });
